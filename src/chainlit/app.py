...
async def on_chat_start():
    ...
    await cl.Message(
        content="Welcome! Type your message or click 🎤 mic button to record voice!",
    ).send()
...
async def on_message(message: cl.Message):
    ...
    try:
        audio_elems = [
            e for e in getattr(message, "elements", [])
            if getattr(e, "mime", "").startswith("audio")
        ]
        ...
        if "application/json" in ctype:
            ...
            add_to_memory(client_id, "user", message.content or "voice input")
            ...
            await cl.Message(
                author="Assistant",
                elements=[
                    cl.Audio(name="Response Audio", content=resp.content, mime_type=ctype, auto_play=True)
                ]
            ).send()
